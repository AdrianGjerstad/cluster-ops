...
class Worker extends EventEmitter {
  ...
  constructor() {
    ...
    for (let i = 0; i < Worker.CATCH_SIGNALS.length; ++i) {
      process.on(`SIG${Worker.CATCH_SIGNALS[i]}`, this.#onExit.bind(this, {
        signal: `SIG${Worker.CATCH_SIGNALS[i]}`
      }, true));
    }

    // If we lose the IPC channel, this worker is as good as dead.
    process.on("disconnect", this.#onExit.bind(this, {
      disconnected: true
    }, true));

    process.on("uncaughtExceptionMonitor", this.#onExit.bind(this, {
      error: true
    }, false));
  }
